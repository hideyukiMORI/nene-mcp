...
ft_topic() {
  ...
  if (( n == 450 )); then
    ...
    case $(( n % 6 )) in
      0) echo "${base} + getTodoById without Bearer (L18)" ;;
      1) echo "${base} + createTodo invalid Bearer (L18)" ;;
      2) echo "${base} + getTodoById not found (L18)" ;;
      3) echo "${base} + createTodo missing title (L18)" ;;
      4) echo "${base} + Bearer health then list (L18)" ;;
      5) echo "${base} + NENE2 catalog + getTodoById (L18)" ;;
    esac
  elif (( n >= 720 )); then
    case $(( n % 6 )) in
    ...
    case $(( n % 6 )) in
      ...
      1) echo "${base} + empty title rejected (L16)" ;;
      2) echo "${base} + public health without Bearer (L16)" ;;
      3) echo "${base} + Bearer + min timeout (L16)" ;;
      4) echo "${base} + sequential list/create (L16)" ;;
      5) echo "${base} + long title write (L16)" ;;
    esac
  elif (( n >= 660 )); then
    case $(( n % 6 )) in
      0) echo "${base} + unicode Bearer write (L15)" ;;
      1) echo "${base} + create/read round-trip (L15)" ;;
      2) echo "${base} + Bearer + timeout composite (L15)" ;;
      3) echo "${base} + NENE2 base + Bearer (L15)" ;;
      4) echo "${base} + Bearer + stderr log write (L15)" ;;
      5) echo "${base} + sessionLogin fail-closed (L15)" ;;
    esac
  elif (( n >= 630 )); then
    case $(( n % 6 )) in
    ...
    case $(( n % 6 )) in
      ...
      1) echo "${base} + float timeout fallback (L13)" ;;
      2) echo "${base} + negative timeout fallback (L13)" ;;
      3) echo "${base} + catalog env precedence (L13)" ;;
      4) echo "${base} + write fail-closed + timeout (L13)" ;;
      5) echo "${base} + HTTPS TLS CA fail-fast (L13)" ;;
    esac
  elif (( n >= 570 )); then
    case $(( n % 6 )) in
...
l13_probe() {
  ...
  local tmp="$2"
  local variant=$(( n % 6 ))
  local dir="/tmp/ft-l13-${n}"
  local cat partial out err rc=0

  echo "" >>"$tmp"
  echo "# L13 probe (FT600+, HTTP diagnostics, variant ${variant})" >>"$tmp"

  unset NENE_MCP_BEARER_TOKEN NENE2_LOCAL_TOOLS_JSON NENE2_LOCAL_API_BASE_URL
  ...
  case "$variant" in
    0)
      ...
      echo "--- stderr ---" >>"$tmp"
      cat "$err" >>"$tmp"
      if grep -q '\[nene-mcp\]' "$err" && grep -q 'status=' "$err" && ! grep -qi 'bearer\|secret\|token' "$err"; then
        echo "ADV-PASS stderr HTTP log line; no secret in stderr" >>"$tmp"
      else
        echo "FINDING (L13-1): stderr HTTP log missing or leaked secret" >>"$tmp"
        rc=1
      fi
      rm -f "$err"
      ;;
    1)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:9090
      unset NENE_MCP_TOOLS_JSON NENE_MCP_TLS_CA_FILE NENE_MCP_LOG
      export NENE_MCP_HTTP_TIMEOUT_SEC=10.5
      out="$(printf '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"nene_mcp_about","arguments":{}}}\n' \
        | php "$ROOT/bin/nene-mcp" 2>/dev/null)"
      echo "$out" >>"$tmp"
      echo "$out" | grep -q '"httpTimeoutSec":10' && echo "ADV-PASS float timeout falls back to 10" >>"$tmp" || { echo "FINDING (L13-2): float timeout not rejected" >>"$tmp"; rc=1; }
      ;;
    2)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:9090
      unset NENE_MCP_TOOLS_JSON NENE_MCP_TLS_CA_FILE NENE_MCP_LOG
      export NENE_MCP_HTTP_TIMEOUT_SEC=-5
      out="$(printf '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"nene_mcp_about","arguments":{}}}\n' \
        | php "$ROOT/bin/nene-mcp" 2>/dev/null)"
      echo "$out" >>"$tmp"
      echo "$out" | grep -q '"httpTimeoutSec":10' && echo "ADV-PASS negative timeout falls back to 10" >>"$tmp" || { echo "FINDING (L13-3): negative timeout not rejected" >>"$tmp"; rc=1; }
      ;;
    3)
      partial="/home/xi/docker/nene-mcp-FT/ft206-persona-bearer-native/docs/mcp/tools-partial.json"
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:9090
      export NENE_MCP_TOOLS_JSON="$FT5_CATALOG"
      export NENE2_LOCAL_TOOLS_JSON="$partial"
      unset NENE_MCP_HTTP_TIMEOUT_SEC NENE_MCP_TLS_CA_FILE NENE_MCP_LOG
      out="$(printf '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}\n' \
        | php "$ROOT/bin/nene-mcp" 2>/dev/null)"
      echo "$out" >>"$tmp"
      echo "$out" | grep -q 'getHealthCheck' && echo "ADV-PASS NENE_MCP_TOOLS_JSON overrides NENE2 catalog alias" >>"$tmp" || { echo "FINDING (L13-4): catalog precedence wrong" >>"$tmp"; rc=1; }
      ;;
    4)
      cat="$(adv_catalog "$dir" write.json '{"tools":[{"name":"w","title":"w","description":"w","safety":"write","source":{"type":"openapi","operationId":"w","method":"POST","path":"/todos"},"inputSchema":{"type":"object","properties":{},"additionalProperties":false},"responseSchemaRef":null}]}')"
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:9090
      unset NENE_MCP_BEARER_TOKEN
      export NENE_MCP_HTTP_TIMEOUT_SEC=45
      unset NENE_MCP_TLS_CA_FILE NENE_MCP_LOG
      out="$(mcp_json "$cat" "tools/call" '{"name":"w","arguments":{}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -q 'bearer authentication' && echo "ADV-PASS write fail-closed holds with custom timeout" >>"$tmp" || { echo "FINDING (L13-5): write bypass with operator timeout" >>"$tmp"; rc=1; }
      ;;
    5)
      export NENE_MCP_API_BASE_URL=https://127.0.0.1:9090
      export NENE_MCP_TOOLS_JSON="$FT5_CATALOG"
      export NENE_MCP_TLS_CA_FILE=/nonexistent/ft-l13-ca.pem
      unset NENE_MCP_HTTP_TIMEOUT_SEC NENE_MCP_LOG NENE_MCP_BEARER_TOKEN
      out="$(printf '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"getHealthCheck","arguments":{}}}\n' \
        | php "$ROOT/bin/nene-mcp" 2>&1 || true)"
      echo "$out" >>"$tmp"
      echo "$out" | grep -q 'TLS CA bundle' && echo "ADV-PASS unreadable TLS CA fails before HTTPS request" >>"$tmp" || { echo "FINDING (L13-6): TLS CA fail-fast missing" >>"$tmp"; rc=1; }
      ;;
  esac
...
l15_probe() {
  ...
  local cat="/home/xi/docker/nene-mcp-FT/ft204-persona-business-hard/docs/mcp/tools.json"
  local token="${NENE_FT450_BEARER_TOKEN:-demo-agent-token}"
  local out err rc=0 new_id

  echo "" >>"$tmp"
  echo "# L15 probe (FT660+, Bearer+operator composite, variant ${variant})" >>"$tmp"
  ...
  case "$variant" in
    0)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:8080
      out="$(mcp_json "$cat" "tools/call" '{"name":"createTodo","arguments":{"title":"L15 unicode テスト"}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*200' && echo "ADV-PASS unicode title createTodo via Bearer" >>"$tmp" || { echo "FINDING (L15-1): unicode write failed" >>"$tmp"; rc=1; }
      ;;
    1)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:8080
      out="$(mcp_json "$cat" "tools/call" '{"name":"createTodo","arguments":{"title":"L15 roundtrip"}}')"
      echo "$out" >>"$tmp"
      new_id="$(echo "$out" | grep -oE '"id"[[:space:]]*:[[:space:]]*[0-9]+' | tail -1 | grep -oE '[0-9]+' || true)"
      if [[ -z "$new_id" ]]; then
        echo "FINDING (L15-2): createTodo did not return id" >>"$tmp"
        rc=1
      else
        out="$(mcp_json "$cat" "tools/call" "{\"name\":\"getTodoById\",\"arguments\":{\"id\":\"id_${new_id}\"}}")"
        echo "$out" >>"$tmp"
        echo "$out" | grep -q 'L15 roundtrip' && echo "ADV-PASS create/read round-trip id_${new_id}" >>"$tmp" || { echo "FINDING (L15-2): read-back failed" >>"$tmp"; rc=1; }
      fi
      ;;
    2)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:8080
      ...
      out="$(mcp_json "$cat" "tools/call" '{"name":"listTodos","arguments":{}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*200' && echo "ADV-PASS Bearer listTodos with timeout=45" >>"$tmp" || { echo "FINDING (L15-3): Bearer+timeout list failed" >>"$tmp"; rc=1; }
      ;;
    3)
      unset NENE_MCP_API_BASE_URL
      export NENE2_LOCAL_API_BASE_URL=http://127.0.0.1:8080
      out="$(mcp_json "$cat" "tools/call" '{"name":"listTodos","arguments":{}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*200' && echo "ADV-PASS NENE2 base alias + Bearer listTodos" >>"$tmp" || { echo "FINDING (L15-4): NENE2+bearer failed" >>"$tmp"; rc=1; }
      ;;
    4)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:8080
      export NENE_MCP_LOG=stderr
      err="$(mktemp)"
      out="$(printf '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"createTodo","arguments":{"title":"L15 stderr write"}}}\n' \
        | NENE_MCP_TOOLS_JSON="$cat" php "$ROOT/bin/nene-mcp" 2>"$err" || true)"
      echo "$out" >>"$tmp"
      cat "$err" >>"$tmp"
      if echo "$out" | grep -qE '"statusCode":\s*200' && grep -q '\[nene-mcp\]' "$err" && ! echo "$out" | grep -q '\[nene-mcp\]'; then
        echo "ADV-PASS Bearer write with stderr log; stdout clean" >>"$tmp"
      else
        echo "FINDING (L15-5): Bearer+stderr write probe failed" >>"$tmp"
        rc=1
      fi
      rm -f "$err"
      ;;
    5)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:8080
      unset NENE_MCP_BEARER_TOKEN
      out="$(mcp_json "$cat" "tools/call" '{"name":"sessionLogin","arguments":{"user_id":"admin","user_pass":"admin"}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -q 'bearer authentication' && echo "ADV-PASS sessionLogin write fail-closed without Bearer" >>"$tmp" || { echo "FINDING (L15-6): sessionLogin bypass without bearer" >>"$tmp"; rc=1; }
      ;;
  esac
...
l16_probe() {
  ...
  local cat="/home/xi/docker/nene-mcp-FT/ft204-persona-business-hard/docs/mcp/tools.json"
  local token="${NENE_FT450_BEARER_TOKEN:-demo-agent-token}"
  local out rc=0 long_title

  echo "" >>"$tmp"
  echo "# L16 probe (FT690+, NeNe observability, variant ${variant})" >>"$tmp"
  ...
  case "$variant" in
    0)
      ...
      out="$(mcp_json "$cat" "tools/call" '{"name":"listTodos","arguments":{}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"requestId":\s*"[0-9a-f]{16,}"' && echo "ADV-PASS NeNe requestId present on Bearer read" >>"$tmp" || { echo "FINDING (L16-1): requestId missing" >>"$tmp"; rc=1; }
      ;;
    1)
      export NENE_MCP_BEARER_TOKEN="$token"
      out="$(mcp_json "$cat" "tools/call" '{"name":"createTodo","arguments":{"title":""}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*400' && echo "ADV-PASS empty title rejected with 400" >>"$tmp" || { echo "FINDING (L16-2): empty title not rejected" >>"$tmp"; rc=1; }
      ;;
    2)
      unset NENE_MCP_BEARER_TOKEN
      out="$(mcp_json "$cat" "tools/call" '{"name":"getHealthCheck","arguments":{}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*200' && echo "ADV-PASS public health without Bearer" >>"$tmp" || { echo "FINDING (L16-3): health requires Bearer" >>"$tmp"; rc=1; }
      ;;
    3)
      export NENE_MCP_BEARER_TOKEN="$token"
      export NENE_MCP_HTTP_TIMEOUT_SEC=1
      out="$(mcp_json "$cat" "tools/call" '{"name":"listTodos","arguments":{}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*200' && echo "ADV-PASS Bearer listTodos with timeout=1" >>"$tmp" || { echo "FINDING (L16-4): min timeout broke Bearer read" >>"$tmp"; rc=1; }
      ;;
    4)
      export NENE_MCP_BEARER_TOKEN="$token"
      out="$(mcp_json "$cat" "tools/call" '{"name":"listTodos","arguments":{}}')"
      echo "$out" >>"$tmp"
      out="$(mcp_json "$cat" "tools/call" '{"name":"createTodo","arguments":{"title":"L16 sequential"}}')"
      echo "$out" >>"$tmp"
      if echo "$out" | grep -qE '"statusCode":\s*200'; then
        echo "ADV-PASS sequential list then create both succeed" >>"$tmp"
      else
        echo "FINDING (L16-5): sequential Bearer calls failed" >>"$tmp"
        rc=1
      fi
      ;;
    5)
      export NENE_MCP_BEARER_TOKEN="$token"
      long_title="L16$(printf '%0.sx' {1..196})"
      out="$(mcp_json "$cat" "tools/call" "{\"name\":\"createTodo\",\"arguments\":{\"title\":\"${long_title}\"}}")"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*200' && echo "ADV-PASS long title (200 chars) accepted" >>"$tmp" || { echo "FINDING (L16-6): long title write failed" >>"$tmp"; rc=1; }
      ;;
  esac
  unset NENE_MCP_HTTP_TIMEOUT_SEC NENE_MCP_TLS_CA_FILE NENE_MCP_LOG NENE2_LOCAL_API_BASE_URL
  unset NENE_MCP_BEARER_TOKEN NENE_MCP_TOOLS_JSON
...
l18_probe() {
  ...
  echo "" >>"$tmp"
  echo "# L18 probe (FT750+, Bearer read-path hardening, variant ${variant})" >>"$tmp"
  ...
  case "$variant" in
    0)
      ...
      out="$(mcp_json "$cat" "tools/call" '{"name":"getTodoById","arguments":{"id":"id_1"}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*401' && echo "ADV-PASS getTodoById without Bearer returns 401" >>"$tmp" || { echo "FINDING (L18-1): unauthenticated getTodoById not blocked" >>"$tmp"; rc=1; }
      ;;
    1)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:8080
      export NENE_MCP_BEARER_TOKEN=invalid-ft750-token
      out="$(mcp_json "$cat" "tools/call" '{"name":"createTodo","arguments":{"title":"L18 invalid bearer"}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*401' && echo "ADV-PASS createTodo invalid Bearer returns 401" >>"$tmp" || { echo "FINDING (L18-2): invalid Bearer write not rejected" >>"$tmp"; rc=1; }
      ;;
    2)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:8080
      export NENE_MCP_BEARER_TOKEN="$token"
      out="$(mcp_json "$cat" "tools/call" '{"name":"getTodoById","arguments":{"id":"id_999999"}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*404' && echo "ADV-PASS getTodoById not found returns 404" >>"$tmp" || { echo "FINDING (L18-3): missing todo not 404" >>"$tmp"; rc=1; }
      ;;
    3)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:8080
      export NENE_MCP_BEARER_TOKEN="$token"
      out="$(mcp_json "$cat" "tools/call" '{"name":"createTodo","arguments":{}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*400' && echo "ADV-PASS createTodo missing title returns 400" >>"$tmp" || { echo "FINDING (L18-4): missing title not rejected" >>"$tmp"; rc=1; }
      ;;
    4)
      export NENE_MCP_API_BASE_URL=http://127.0.0.1:8080
      export NENE_MCP_BEARER_TOKEN="$token"
      ...
      out="$(mcp_json "$cat" "tools/call" '{"name":"listTodos","arguments":{}}')"
      echo "$out" >>"$tmp"
      if echo "$out" | grep -qE '"statusCode":\s*200' && echo "$out" | grep -q 'listTodos'; then
        echo "ADV-PASS Bearer health then listTodos both succeed" >>"$tmp"
      else
        echo "FINDING (L18-5): Bearer read chain failed" >>"$tmp"
        rc=1
      fi
      ;;
    5)
      unset NENE_MCP_API_BASE_URL
      export NENE2_LOCAL_API_BASE_URL=http://127.0.0.1:8080
      export NENE2_LOCAL_TOOLS_JSON="$cat"
      export NENE_MCP_BEARER_TOKEN="$token"
      out="$(mcp_json "$cat" "tools/call" '{"name":"getTodoById","arguments":{"id":"id_1"}}')"
      echo "$out" >>"$tmp"
      echo "$out" | grep -qE '"statusCode":\s*200' && echo "ADV-PASS NENE2 catalog alias + Bearer getTodoById" >>"$tmp" || { echo "FINDING (L18-6): NENE2 catalog getTodoById failed" >>"$tmp"; rc=1; }
      ;;
  esac
  unset NENE_MCP_HTTP_TIMEOUT_SEC NENE_MCP_TLS_CA_FILE NENE_MCP_LOG NENE2_LOCAL_TOOLS_JSON NENE2_LOCAL_API_BASE_URL
  unset NENE_MCP_BEARER_TOKEN NENE_MCP_TOOLS_JSON
...
write_report() {
  ...
  if echo "$output" | grep -q 'FINDING (F-'; then
    ...
    friction_block="L18 + L17 … L6 adversarial exercised — Bearer read-path hardening band."
  elif (( n >= 720 )); then
    ...
    friction_block="L15 + L14 … L6 adversarial exercised — Bearer+operator composite probes."
  elif (( n >= 630 )); then
